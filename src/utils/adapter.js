...
const adaptEditorOfferToClient = (offer, isNewData) => {
  return {
...
const adaptEditorDestinationToClient = (destination) => {
  return {
...
export const adaptEditorOffersToClient = (offers, isNewData = true) => {
  const adaptedOffers = [];

  offers.forEach((offer) => {
    adaptedOffers.push(adaptEditorOfferToClient(offer, isNewData));
  });
  return adaptedOffers;
};

export const adaptEditorDestinationsToClient = (destinations) => {
  const adaptedDestinations = [];

  destinations.forEach((destination) => {
    adaptedDestinations.push(adaptEditorDestinationToClient(destination));
  });
